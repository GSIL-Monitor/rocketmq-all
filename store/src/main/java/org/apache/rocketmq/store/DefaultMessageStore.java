...
public class DefaultMessageStore implements MessageStore {
     ...
     * 4、 启动拉消息管理服务 PullRequestHoldService，当拉取消息时未发现消息，则初始化 PullRequeset 对象放入该服务线程的 pullRequestTable 列表中，
     * 由 PullRequestHoldService 每隔 1 秒钟就检查一遍每个 PullRequeset 对象要读取的数据位置在 consumequeue 中是否已经有数据了，若有则交由PullManageProcessor 处理。
...
//                    }
     ...
     * 2）从获得的 Map 对象中根据 queueId 获得 ConsumeQueue 对象，
     * 若为空，则创建 ConsumeQueue 对象，在创建该对象时初始化了 topic、queueId、storePath、mappedFileSize 等变量；
     * 也初始化了 MappedFileQueue 对象； 将初始化的ConsumeQueue 对象存入 Map 中并返回；若不为空则直接返回。
     */
