...
public class CommitLog {
    ...
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            // 从 MappedFileQueue 的 MappedFile 列表的倒数第三个对象（即倒数第三个文件）开始遍历每块消息单元，
            ...
            while (true) {
                ...
                // Normal data 对于 msgSize 大于零，则读取的偏移量 mappedFileOffset 累加 msgSize；
                ...
                //若等于零，则表示读取到了文件的最后一块信息，则继续读取下一个 MappedFile
            ...
            //计算有效信息的最后位置 processOffset，取最后读取的MappedFile 对象的 fileFromOffset 加上最后读取的位置 mappedFileOffset 值。
    ...
    /**
     *  异常恢复CommitLog内存数据
     * 与正常恢复的区别在于: 正常恢复是从倒数第 3 个文件开始恢复；
     * 而异常恢复是从最后的文件开始往前寻找与checkpoint 文件的记录相匹配的一个文件
     */
    ...
    /**
     * 1、从 MappedFile 列表中的最后一个对象开始往前遍历每个 MappedFile 对象，检查该 MappedFile 对象对应的文件是否满足恢复条件，
     * 查找逻辑如下，若查找完整个队列未找到符合条件的 MappedFile 对象，则从第一个文件开始恢复。
     * A） 从该文件中获取第一个消息单元的第 5 至 8 字节的 MAGICCODE 字段，若该字段等于 MessageMagicCode（即不是正常的消息内容），
     * 则直接返回后继续检查前一个文件；
     * B）获取第一个消息单元的第 56 位开始的 8 个字节的 storeTimeStamp 字段，若等于零，也直接返回后继续检查前一个文件；
     * C）检查是否开启消息索引功能 （ MessageStoreConfig .messageIndexEnable，默认为 true）
     * 并且是否使用安全的消息索引功能（ MessageStoreConfig.MessageIndexSafe，默认为 false，在可靠模式下， 异常宕机恢复慢；
     * 非可靠模式下，异常宕机恢复快） ，若开启可靠模式下面的消息索引，则消息的storeTimeStamp 字段表示的时间戳必须小于 checkpoint
     * 文件中物理队列消息时间戳、逻辑队列消息时间戳、索引队列消息时间戳这三个时间戳中最小值，才满足恢复数据的条件；
     * 否则消息的 storeTimeStamp 字段表示的时间戳必须小于
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            //消息内容写入 MappedFile.mappedByteBuffer： MappedByteBuffer 对象，
            //即写入消息缓存中；由后台服务线程定时的将缓存中的消息刷盘到物理文件中
            result = mappedFile.appendMessage(msg, this.appendMessageCallback);
            /**
             * 若最后一个 MappedFile 剩余空间不足够写入此次的消息内容，即返回状态为
             * END_OF_FILE 标记， 则再次调用 MapedFileQueue.getLastMappedFile 方法获取新
             * 的 MappedFile 对象然后调用 MappedFile.appendMessage 方法重写写入，最后继续
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                ...
                //该线程的 doCommit 方法中遍历读队列的数据，检查MappedFileQueue.committedWhere（刷盘刷到哪里的记录）
     ...
     * 1）获取当前内存对象的写入位置（ wrotePosition 变量值）；若写入位置没有超过文件大小则继续顺序写入；
     * 2）由内存对象 mappedByteBuffer 创建一个指向同一块内存的 ByteBuffer对象，并将内存对象的写入指针指向写入位置；
     * 3）以文件的起始偏移量（ fileFromOffset）、 ByteBuffer 对象、该内存对象剩余的空间（ fileSize-wrotePosition）、消息对象 msg 为参数调用
     */
