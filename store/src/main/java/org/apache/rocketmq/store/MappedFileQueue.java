...
// 在应用层中访问commitlog和consumequeue是通过MappFileQueue来操作MappedFile类的，
// 从而间接操作磁盘上面的文件，MappFileQueue是多个MappedFile组成的队列
public class MappedFileQueue {
    ...
    /**
     *  清理指定偏移量所在文件之后的文件
     * 遍历整个列表，每个MappedFile对象对应着一个固定大小的文件，当文件的起始偏移量fileFromOffset<=offset<=fileFromOffset+fileSize，
     * 则表示指定的位置偏移量 offset 落在的该文件上，则将对应的 MapedFile 对象的 wrotepostion 和commitPosition 设置为 offset%fileSize，
    ...
    public MappedFile getLastMappedFile(final long startOffset, boolean needCreate) {
        ...
        // 若列表为空，则创建的新文件的文件名（即 fileFromOffset 值）为 0
        // 若最后一个文件写满，则新文件的文件名等于最后一个文件的fileFromOffset+fileSize
