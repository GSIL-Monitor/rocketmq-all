...
public class TransactionalMessageServiceImpl implements TransactionalMessageService {
    ...
    private boolean putBackHalfMsgQueue(MessageExt msgExt, long offset) {
        ...
        // 为什么需要这样呢，答案在listener.resolveHalfMsg(msgExt)中。
    ...
    public void check(long transactionTimeout, int transactionCheckMax,
        ...
        try {
            ...
            for (MessageQueue messageQueue : msgQueues) {
                 ...
                 * 然后获取操作队列的消费进度、待操作的消费队列的消费进度，如果任意一小于0，忽略该消息队列，继续处理下一个队列
                 */
                 ...
                 *   这里说一下RMQ_SYS_TRANS_HALF_TOPIC、RMQ_SYS_TRANS_OP_HALF_TOPIC这两个主题的作用。
                ...
                // single thread
                int getMessageNullCount = 1; // getMessageNullCount: 获取空消息的次数
                long newOffset = halfOffset; //newOffset: 当前处理RMQ_SYS_TRANS_HALF_TOPIC#queueId的最新进度
                long i = halfOffset;
                while (true) {
                    /**
                     * 就是一个任务处理，可以限制每次最多处理的时间，RocketMQ为待检测主题RMQ_SYS_TRANS_HALF_TOPIC的每个队列，
                    ...
                    if (removeMap.containsKey(i)) {
                         ...
                         *  该时间就是假设事务消息发送成功后，应用程序事务提交的时间，在这段时间内，RocketMQ任务事务未提交，故不应该在这个时间段向应用程序发送回查请求。
