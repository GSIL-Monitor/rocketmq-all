 ...
 * PullMessageService线程的LinkedBlockingQueue,进而唤醒 queue.take()方法，然后执行DefaultMQPushConsumerImpl 的 pullMessage,
 * 通过网络从broker端拉取消息，一次最多拉取的消息条数可配置，默认为1条，然后然后将拉取的消息，执行过滤等，然后封装成任务（ConsumeRequest）,
 * 提交到消费者的线程池去执行，每次消费消息后，又将该PullRequest放入到PullMessageService中（DefaultMQPushConsumerImpl 的机制就是pullInterval - 0；
 */
public abstract class RebalanceImpl {
    ...
    public void doRebalance(final boolean isOrder) {
        ...
        //移除MessageQueu，如果MesageQueue的topic不在订阅的主题中，接下来重点关注rebalanceByTopic方法
    ...
    private boolean updateProcessQueueTableInRebalance(final String topic, final Set<MessageQueue> mqSet,
        ...
        while (it.hasNext()) {
            ...
            if (mq.getTopic().equals(topic)) {
                /** 不在参数的集合中
                 * 遍历消息队列-处理队列缓存，只处理mq的主题与该主题相关的ProcessQueue,如果mq不在当期主题的处理范围内
                 * （由于消息队列数量变化等原因，消费者的消费队列发生了变化，该消息队列已经分配给别的消费者去消费了），
                 * 首先设置该消息队列为丢弃(dropped为voliate修饰)，可以及时的阻止继续向ProceeQueue中拉取数据，
                 */
                ...
                } else if (pq.isPullExpired()) { //如果距离上次拉取时间已经超时, 默认超时12s
        ...
        /**
         * 处理MessageQueue的ProcessQueue,也就是在ProcessQueueTable中没有mq的处理队列（因为重新负载后，可能会分配一些新的队列）
         */
        ...
        //目的: 将 PullRequest 对象放入 PullMessageService服务线程的 pullRequestQueue 队列中
