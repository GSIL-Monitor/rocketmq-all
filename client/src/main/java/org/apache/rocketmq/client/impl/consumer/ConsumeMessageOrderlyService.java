...
/**
 * 1) ConsumeMessageOrderlyService 类中的定时任务会每隔20秒钟执行一次lockMQPeriodically方法，
 *  获取该Consumer客户端在Broker端锁住的MessageQueue集合（分布式锁），并通过该集合获取对应的ProcessQueue（消息处理队列）
 *  加上本地锁和加锁时间，目的是为了消费时在本地检查消费队列是否被锁住。
 * 2)在进行消息队列的消费过程中，对MessageQueue对象进行本地同步锁，保证同一个时间只允许一个线程消息一个ConsumeQueue队列。
 * 3) 在回调业务层定义ConsumeMessageOrderlyService.messageListener:MessageListenerOrderly
 *  类的 consumeMessage 方法之前获取 ProcessQueue.lockConsume:ReentrantLock变量的锁即消费处理队列的锁，
 *  该锁的粒度比消息队列的同步锁粒度更小。
 */
            ...
            synchronized (objLock) {
                ...
                if (MessageModel.BROADCASTING.equals(ConsumeMessageOrderlyService.this.defaultMQPushConsumerImpl.messageModel())
                    ...
                    for (boolean continueConsume = true; continueConsume; ) {
                        ...
                        //takeMessages从ProcessQueue.msgTreeMap变量中获取 batchSize个数的 List<MessageExt>列表；
                        // 并且从 msgTreeMap 中删除，存入临时变量 msgTreeMapTemp 中
