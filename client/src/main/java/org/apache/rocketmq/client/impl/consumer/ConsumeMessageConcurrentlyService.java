...
public class ConsumeMessageConcurrentlyService implements ConsumeMessageService {
    ...
    public ConsumeMessageConcurrentlyService(DefaultMQPushConsumerImpl defaultMQPushConsumerImpl,
        ...
        this.consumeExecutor = new ThreadPoolExecutor(
            this.defaultMQPushConsumer.getConsumeThreadMin(), //线程池的常驻线程数：consumeThreadMin
            this.defaultMQPushConsumer.getConsumeThreadMax(), // 线程池的最大线程数：consumeThreadMax
            1000 * 60,
    ...
    public void processConsumeResult(
        ...
        switch (status) {
            //1)消费成功， 对于批次消费消息，返回消费成功并不代表所有消息都消费成功
            case CONSUME_SUCCESS:
                ...
                break;
            //2)但是消费消息的时候一旦遇到消费消息失败直接放回，根据 ackIndex 来标记
            case RECONSUME_LATER:
                ackIndex = -1;
        ...
        switch (this.defaultMQPushConsumer.getMessageModel()) {
            ...
            case BROADCASTING:
                ...
                break;
            /**
             * 集群模式: 将消费失败的消息一条条的发送到 broker 的重试队列中去，如果此时还有发送到重试队列发送失败的消息，
             *  那就在 consumer 的本地线程定时 5秒钟以后重试重新消费消息， 在走一次上面的消费流程
             */
            case CLUSTERING:
    ...
    class ConsumeRequest implements Runnable {
        ...
        public void run() {
            ...
            try {
                //给消息设置消费失败时候的 retry topic，当消息发送失败的时候发送到 topic为%RETRY%groupname 的队列中
