 ...
 * 4、RocketMQ 推拉模式实现机制
 * 5: 1个消费者可以消费多个消息队列，但一个消息队列同一时间只能被一个消费者消费，这又是如何实现的呢？
 *
 * ---------------------
 * 我们知道，一个应用程序（消费端），一个消费组 一个 DefaultMQPushConsumerImpl ，同一个IP:端口，会有一个MQClientInstance ，
 * 而每一个MQClientInstance中持有一个PullMessageServive实例，故可以得出如下结论：
 * 同一个应用程序中，如果存在多个消费组，那么多个DefaultMQPushConsumerImpl 的消息拉取，都需要依靠一个PullMessageServive
 *
 * 那他们之间又是如何协作的呢？
...
public class DefaultMQPushConsumerImpl implements MQConsumerInner {
    ...
    /**
     * MQClientInstance start --> (this.rebalanceService.start())
    ...
    /**
     * MQClientInstance mq客户端实例，每台应用服务器将持有一个MQClientInstance对象，
     * 供该应用服务器的消费者，生产者使用。该类是消费者，生产者网络处
     */
    private MQClientInstance mQClientFactory;
    /**
     * 其实就只有Pull模式，所以Push其实就是用pull封装一下
     */
    ...
    /**
     *  Push模式下的消息消费 : 拉取消息（pullMessage）
     *  消息的消费过程，就是从服务器拉取，然后消费者进行消费，再根据业务反馈是否成功消费来推动消费进度，
     *  也就是消息的消费进度并不是保存在服务端（比如commitlog文件中），而是保存在消费端（可以是本地（广播模式）、broke端（集群模式））
     */
    public void pullMessage(final PullRequest pullRequest) {
        ...
        if (this.isPause()) {
            ...
            //1 秒之后再次将该 PullRequest 对象放入 PullMessageService.pullRequestQueue 队列中
        ...
        if (!this.consumeOrderly) {
            ...
            //Concurrently max span offset , default is 2000
                    ...
                    switch (pullResult.getPullStatus()) {
                        case FOUND:
                            ...
                            if (pullResult.getMsgFoundList() == null || pullResult.getMsgFoundList().isEmpty()) {
                                 ...
                                 * 3、分页消费（每次传给业务消费监听器的最大数量为配置的consumeMessageBatchMaxSize
                                 * 4、执行消费后钩子函数，并根据业务方返回的消息消费结果（成功，重试）【ACK】确认信息，然后更新消息进度，从ProceeQueue中删除相应的消息
                                 * ---------------------
                                 * 2）顺序消息 消息消费服务ConsumeMessageService的提交消费请求【ConsumeMessageOrderlyService】
                                 *   这里与非顺序消息的区别是ConsumeRequest只针对ProcessQueue,messageQueue
                                 */
        ...
        try {
            this.pullAPIWrapper.pullKernelImpl(
                ...
                subscriptionData.getExpressionType(), //expressionTYpe
                subscriptionData.getSubVersion(), //版本
                pullRequest.getNextOffset(), // pullRequest.getNextOffset()
    ...
    public synchronized void start() throws MQClientException {
        ...
        switch (this.serviceState) {
            ...
            case CREATE_JUST:
                ...
                /*
                    ConsumeMessageConcurrentlyService 类的 start 方法 设置定时器,cleanExpireMsg
        ...
        /*
            第一，向所有在 MQClientInstance.brokerAddrTable 列表中的 Broker 发送心跳消息；
            第二，向 Filter 过滤服务器发送REGISTER_MESSAGE_FILTER_CLASS 请求码，更新过滤服务器中的 Filterclass 文件；
        */
