...
/**
 * 设置两个定时任务
 * （1）每隔十秒钟遍历brokerLiveTable集合，查看每个broker的最后更新时间是否超过了两分钟，
 * 超过则关闭broker的渠道并清理 RouteInfoManager 类的topicQueueTable、 brokerAddrTable、
 * clusterAddrTable、 filterServerTable成员变量
...
public class RouteInfoManager {
    ...
    /**
     *  主题与topic对应关系,topic.json的topicConfigTable的数据在QueueData对象中记录了该topic的对应brokerName
     ...
     * 消息发送者是否提供了消息重试机制，待后续文章分析broker时再研究。
     */
        ...
        try {
            try {
                ...
                if (null != topicConfigWrapper
                    ...
                    //根据 NameServer 存储的 Broker 版本信息来判断是否需要更新 NameServer 端的 topic 配置信息
